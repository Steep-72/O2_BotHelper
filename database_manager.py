...
def save_notification_to_db(user_id, company, product, expiry_date, notify_date, quantity, notification_type):
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    try:
        cursor.execute('''
...
def get_notifications_from_db():
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute('SELECT id, user_id, company, product, expiry_date, notify_date, quantity, notification_type FROM notifications')
...
def add_monitored_site(site):
    ...
    cursor = conn.cursor()
    try:
        cursor.execute('INSERT INTO monitored_sites (site) VALUES (?)', (site,))
        conn.commit()
        return True
    except sqlite3.IntegrityError:
        return False
...
def remove_monitored_site(site):
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute('DELETE FROM monitored_sites WHERE site = ?', (site,))
    conn.commit()
    conn.close()
...
def get_access_request_info(user_id):
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute('SELECT username, first_name, last_name FROM access_requests WHERE user_id = ?', (user_id,))
    result = cursor.fetchone()
    conn.close()
    return result
